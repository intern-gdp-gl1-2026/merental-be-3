...
def request_json(url, token, retries=1):
    ...
    attempt = 0
    # Allow skipping SSL verification when SENTRY_INSECURE env var is set (1/true/yes)
    insecure = os.environ.get("SENTRY_INSECURE", "").lower() in {"1", "true", "yes"}
    ssl_context = None
    if insecure:
        try:
            import ssl
            ...
            # If ssl isn't available for some reason, fall back to default behavior and let it fail
            ssl_context = None
